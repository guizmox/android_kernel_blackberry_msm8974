...
#define POWER_ACC	(1 << 0)
#define POWER_MAG	(1 << 1)
...
#ifdef CONFIG_PM
	atomic_t enable_acc_on_resume;
	atomic_t enable_mag_on_resume;
#endif /* CONFIG_PM */
...
static int lsm303d_config_regulator(struct lsm303d_status *stat, bool on, u8 vote);
...
static const struct lsm303d_acc_platform_data default_lsm303d_acc_pdata = {
	...
	.gpio_int1 = 73,
	.gpio_int2 = 65,
	.power_on = lsm303d_config_regulator,
	.power_off = lsm303d_config_regulator,
};
...
static int lsm303d_hw_init(struct lsm303d_status *stat)
{
	...
	if (err < 0) {
		...
		err = lsm303d_i2c_read(stat, buf, 1);
		if(err < 0) {
			dev_warn(&stat->client->dev, "Error retry WHO_AM_I\n");
			goto err_firstread;
		}
		stat->hw_working = 1;
	} else
...
#ifdef CONFIG_PM
	atomic_set(&stat->enable_acc_on_resume, 0);
	atomic_set(&stat->enable_mag_on_resume, 0);
#endif /* CONFIG_PM */
...
struct sensor_regulator lsm303d_vreg[] = {
...
{
	int rc = 0, i;
	int num_reg = sizeof(lsm303d_vreg) / sizeof(struct sensor_regulator);

	if (on) {
		u8 tmp = stat->power_vote;
		stat->power_vote |= vote;
		if (tmp) {
			return 0;
		}
		for (i = 0; i < num_reg; i++) {
			lsm303d_vreg[i].vreg =
				...
				lsm303d_vreg[i].name);
			if (IS_ERR(lsm303d_vreg[i].vreg)) {
				rc = PTR_ERR(lsm303d_vreg[i].vreg);
				pr_err("%s:regulator get failed rc=%d\n",
								__func__,rc);
				lsm303d_vreg[i].vreg = NULL;
				goto error_vdd;
			}

			if (regulator_count_voltages(
				lsm303d_vreg[i].vreg) > 0) {
				rc = regulator_set_voltage(
					lsm303d_vreg[i].vreg,
					lsm303d_vreg[i].min_uV,
					lsm303d_vreg[i].max_uV);
				if (rc) {
					pr_err("%s: set voltage failed rc=%d\n",
					__func__, rc);
					regulator_put(lsm303d_vreg[i].vreg);
					lsm303d_vreg[i].vreg = NULL;
					goto error_vdd;
				}
			}

			rc = regulator_enable(lsm303d_vreg[i].vreg);
			if (rc) {
...
error_vdd:
	while (--i >= 0) {
		if (!IS_ERR_OR_NULL(lsm303d_vreg[i].vreg)) {
			if (regulator_count_voltages(
			lsm303d_vreg[i].vreg) > 0) {
				regulator_set_voltage(lsm303d_vreg[i].vreg,
						0, lsm303d_vreg[i].max_uV);
			}
			regulator_disable(lsm303d_vreg[i].vreg);
			regulator_put(lsm303d_vreg[i].vreg);
			lsm303d_vreg[i].vreg = NULL;
		}
	}
...
static int lsm303d_acc_device_power_off(struct lsm303d_status *stat)
{
	...
	if (stat->pdata_acc->power_off) {
		stat->pdata_acc->power_off(stat, false, POWER_ACC);
	}
...
static int lsm303d_mag_device_power_off(struct lsm303d_status *stat)
{
	...
	if (stat->pdata_mag->power_off) {
		stat->pdata_mag->power_off(stat, false, POWER_MAG);
	}
...
static int lsm303d_acc_device_power_on(struct lsm303d_status *stat)
{
	...
	if (stat->pdata_acc->power_on) {
		err = stat->pdata_acc->power_on(stat, true, POWER_ACC);
		if (err < 0) {
			dev_err(&stat->client->dev,
...
static int lsm303d_mag_device_power_on(struct lsm303d_status *stat)
{
	...
	if (stat->pdata_mag->power_on) {
		err = stat->pdata_mag->power_on(stat, true, POWER_MAG);
		if (err < 0) {
			dev_err(&stat->client->dev,
...
enum hrtimer_restart poll_function_read_mag(struct hrtimer *timer)
{
	...
	if (atomic_read(&stat->enabled_mag)) {
		hrtimer_forward_now(&stat->hr_timer_mag, stat->ktime_mag);
		return HRTIMER_RESTART;
	} else {
		return HRTIMER_NORESTART;
	}
		...
		const struct i2c_device_id *id)
{
	...
	lsm303d_config_regulator(stat, true, POWER_ACC|POWER_MAG);
	err = lsm303d_hw_init(stat);
	lsm303d_config_regulator(stat, false, POWER_ACC|POWER_MAG);
...
static int lsm303d_suspend(struct device *dev)
{
	...
	dev_info(&stat->client->dev, "suspend: accel: %d, mag: %d\n",
		atomic_read(&stat->enabled_acc),
		atomic_read(&stat->enabled_mag));
...
static int lsm303d_resume(struct device *dev)
{
	...
	dev_info(&stat->client->dev, "resume: accel: %d, mag: %d\n",
		atomic_read(&stat->enable_acc_on_resume),
		atomic_read(&stat->enable_mag_on_resume));

	if (atomic_cmpxchg(&stat->enable_acc_on_resume, 1, 0)) {
		lsm303d_acc_enable(stat);
	}
...
#ifdef CONFIG_PM
static const struct dev_pm_ops lsm303d_pm = {
