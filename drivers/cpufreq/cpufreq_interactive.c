...
/* screen off wseries power mitigation */
static unsigned int screen_off_cpu_max_freq_val = 652800;
static unsigned long screen_off_throttle_count_val = 0;
...
static void cpufreq_interactive_timer(unsigned long data)
{
	...
	/* Ajustement du timer selon lՎtat de lՎcran */
	...
	//guizmox fix for wseries
	//&& pcpu->policy->cpu == 0 
	int ps = power_supply_is_system_supplied();	
	if (ps == 0)
	{
    	if (!display_on && new_freq > screen_off_cpu_max_freq_val) {
        	new_freq = screen_off_cpu_max_freq_val;
			screen_off_throttle_count_val++;
    	}
	}
	...
	/*
	 * Update timestamps for minimum sample enforcement.
	 */
...
static ssize_t show_screen_off_cpu_max_freq(struct kobject *kobj,
        struct attribute *attr, char *buf)
{
    return sprintf(buf, "%u\n", screen_off_cpu_max_freq_val);
}

static ssize_t show_screen_off_throttle_count(struct kobject *kobj,
    struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%lu\n", screen_off_throttle_count_val);
}

define_one_global_ro(screen_off_throttle_count);

static ssize_t store_screen_off_cpu_max_freq(struct kobject *kobj,
        struct attribute *attr, const char *buf, size_t count)
{
    ...
    screen_off_cpu_max_freq_val = val;
    return count;
}

define_one_global_rw(screen_off_cpu_max_freq);
...
static struct attribute *interactive_attributes[] = {
	...
	&screen_off_cpu_max_freq.attr,
	&screen_off_throttle_count.attr,
	NULL,
};
...
static struct notifier_block cpufreq_interactive_idle_nb = {
 ...
 static struct cpufreq_frequency_table fixed_freq_table[] = {
	{ 0,  300000 },
	{ 1,  652800 },
	{ 2, 1190400 },
	{ 3, 1958400 },
